package application;

import java.text.ParseException;
import java.text.SimpleDateFormat;
import java.time.Instant;
import java.util.Date;
import java.util.TimeZone;

public class Program09Date {

	public static void main(String[] args) throws ParseException {
		SimpleDateFormat sdf1 = new SimpleDateFormat("dd/MM/yyyy");
		SimpleDateFormat sdf2 = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss");
		SimpleDateFormat sdf3 = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss");

		sdf3.setTimeZone(TimeZone.getTimeZone("GMT"));

		Date d1 = sdf1.parse("25/06/2018");
		Date d2 = sdf2.parse("25/06/2018 15:42:37");
		Date sysdate = new Date();

		Date x2 = new Date(System.currentTimeMillis());
		Date x3 = new Date(0L);
		Date x4 = new Date(1000L * 60L * 60L * 5L);

		Date y3 = Date.from(Instant.parse("2018-06-25T15:42:07Z"));

		System.out.println("d1: " + d1);
		System.out.println("d1: " + sdf1.format(d1));
		System.out.println();
		System.out.println("d2: " + d2);
		System.out.println("d2: " + sdf2.format(d2));

		System.out.println();
		System.out.println("Sysdate: " + sysdate);
		System.out.println("Sysdate: " + sdf1.format(sysdate));
		System.out.println("Sysdate: " + sdf2.format(sysdate));

		System.out.println();
		System.out.println("x2: " + sdf2.format(x2));
		System.out.println("x3: " + sdf2.format(x3));
		System.out.println("x4: " + sdf2.format(x4));

		System.out.println();
		System.out.println("y3: " + sdf2.format(y3));

		System.out.println("--------------------------------------");
		System.out.println("Fomatando utilizando TimeZone GTM");

		System.out.println("d1: " + sdf3.format(d1));
		System.out.println("d2: " + sdf3.format(d2));
		System.out.println("Sysdate: " + sdf3.format(sysdate));
		System.out.println("x2: " + sdf3.format(x2));
		System.out.println("x3: " + sdf3.format(x3));
		System.out.println("x4: " + sdf3.format(x4));
		System.out.println("y3: " + sdf3.format(y3));

	}

}
